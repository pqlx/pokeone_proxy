# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: game/request/Skins.proto

import sys
_b=sys.version_info[0]<3 and (lambda x:x) or (lambda x:x.encode('latin1'))
from google.protobuf import descriptor as _descriptor
from google.protobuf import message as _message
from google.protobuf import reflection as _reflection
from google.protobuf import symbol_database as _symbol_database
from google.protobuf import descriptor_pb2
# @@protoc_insertion_point(imports)

_sym_db = _symbol_database.Default()




DESCRIPTOR = _descriptor.FileDescriptor(
  name='game/request/Skins.proto',
  package='PSXAPI.Request615616321',
  syntax='proto2',
  serialized_pb=_b('\n\x18game/request/Skins.proto\x12\x17PSXAPI.Request615616321\"\x07\n\x05Skins')
)
_sym_db.RegisterFileDescriptor(DESCRIPTOR)




_SKINS = _descriptor.Descriptor(
  name='Skins',
  full_name='PSXAPI.Request615616321.Skins',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  options=None,
  is_extendable=False,
  syntax='proto2',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=53,
  serialized_end=60,
)

DESCRIPTOR.message_types_by_name['Skins'] = _SKINS

Skins = _reflection.GeneratedProtocolMessageType('Skins', (_message.Message,), dict(
  DESCRIPTOR = _SKINS,
  __module__ = 'game.request.Skins_pb2'
  # @@protoc_insertion_point(class_scope:PSXAPI.Request615616321.Skins)
  ))
_sym_db.RegisterMessage(Skins)


# @@protoc_insertion_point(module_scope)

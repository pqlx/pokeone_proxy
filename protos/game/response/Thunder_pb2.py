# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: game/response/Thunder.proto

import sys
_b=sys.version_info[0]<3 and (lambda x:x) or (lambda x:x.encode('latin1'))
from google.protobuf import descriptor as _descriptor
from google.protobuf import message as _message
from google.protobuf import reflection as _reflection
from google.protobuf import symbol_database as _symbol_database
from google.protobuf import descriptor_pb2
# @@protoc_insertion_point(imports)

_sym_db = _symbol_database.Default()




DESCRIPTOR = _descriptor.FileDescriptor(
  name='game/response/Thunder.proto',
  package='PSXAPI.Response89190353',
  syntax='proto2',
  serialized_pb=_b('\n\x1bgame/response/Thunder.proto\x12\x17PSXAPI.Response89190353\"\t\n\x07Thunder')
)
_sym_db.RegisterFileDescriptor(DESCRIPTOR)




_THUNDER = _descriptor.Descriptor(
  name='Thunder',
  full_name='PSXAPI.Response89190353.Thunder',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  options=None,
  is_extendable=False,
  syntax='proto2',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=56,
  serialized_end=65,
)

DESCRIPTOR.message_types_by_name['Thunder'] = _THUNDER

Thunder = _reflection.GeneratedProtocolMessageType('Thunder', (_message.Message,), dict(
  DESCRIPTOR = _THUNDER,
  __module__ = 'game.response.Thunder_pb2'
  # @@protoc_insertion_point(class_scope:PSXAPI.Response89190353.Thunder)
  ))
_sym_db.RegisterMessage(Thunder)


# @@protoc_insertion_point(module_scope)

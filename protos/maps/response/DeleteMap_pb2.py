# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: DeleteMap.proto

import sys
_b=sys.version_info[0]<3 and (lambda x:x) or (lambda x:x.encode('latin1'))
from google.protobuf import descriptor as _descriptor
from google.protobuf import message as _message
from google.protobuf import reflection as _reflection
from google.protobuf import symbol_database as _symbol_database
from google.protobuf import descriptor_pb2
# @@protoc_insertion_point(imports)

_sym_db = _symbol_database.Default()




DESCRIPTOR = _descriptor.FileDescriptor(
  name='DeleteMap.proto',
  package='MAPAPI.Response6',
  syntax='proto2',
  serialized_pb=_b('\n\x0f\x44\x65leteMap.proto\x12\x10MAPAPI.Response6\"\x1c\n\tDeleteMap\x12\x0f\n\x07MapName\x18\x01 \x01(\t')
)
_sym_db.RegisterFileDescriptor(DESCRIPTOR)




_DELETEMAP = _descriptor.Descriptor(
  name='DeleteMap',
  full_name='MAPAPI.Response6.DeleteMap',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='MapName', full_name='MAPAPI.Response6.DeleteMap.MapName', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  options=None,
  is_extendable=False,
  syntax='proto2',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=37,
  serialized_end=65,
)

DESCRIPTOR.message_types_by_name['DeleteMap'] = _DELETEMAP

DeleteMap = _reflection.GeneratedProtocolMessageType('DeleteMap', (_message.Message,), dict(
  DESCRIPTOR = _DELETEMAP,
  __module__ = 'DeleteMap_pb2'
  # @@protoc_insertion_point(class_scope:MAPAPI.Response6.DeleteMap)
  ))
_sym_db.RegisterMessage(DeleteMap)


# @@protoc_insertion_point(module_scope)
